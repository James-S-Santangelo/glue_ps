...
rule angsd_asso_freq:
    ...
    output:
        asso = f'{ANGSD_DIR}/asso/allSamples/asso_freq/{{chrom}}_allSamples_asso_freq.lrt0.gz'
    log: f"{LOG_DIR}/angsd_asso/{{chrom}}_angsd_asso_freq.log"
    container: 'library://james-s-santangelo/angsd/angsd:0.938'
    params:
        max_depth = 5225, # Num samples x Mean coverage x 2
        min_ind = 1045, # 50% of Num samples
        out = f'{ANGSD_DIR}/asso/allSamples/asso_freq/{{chrom}}_allSamples_asso_freq'
    threads: 4
    resources:
...
rule angsd_asso_score:
    ...
    input:
        ...
        sites_idx = rules.index_filtered_snps.output
    output:
        asso = f'{ANGSD_DIR}/asso/allSamples/asso_score/{{chrom}}_allSamples_asso_score.lrt0.gz'
    log: f"{LOG_DIR}/angsd_asso/{{chrom}}_angsd_asso_score.log"
    container: 'library://james-s-santangelo/angsd/angsd:0.938'
    params:
        max_depth = 5225, # Num samples x Mean coverage x 2
        min_ind = 1045, # 50% of Num samples
        out = f'{ANGSD_DIR}/asso/allSamples/asso_score/{{chrom}}_allSamples_asso_score'
    threads: 4
    resources:
